...
if 'metrics' not in st.session_state:
    st.session_state.metrics = {"regressions": 0, "fixations": 0, "risk_score": 0, "start_time": None}

# -----------------------------------------------------------------------------
# 3. OPTIMIZED MEDIAPIPE ENGINE (Processes 1 in 10 frames = 3 FPS)
# -----------------------------------------------------------------------------
...
class VideoTransformer(VideoTransformerBase):
    ...
    def recv(self, frame):
        ...
        # CRITICAL OPTIMIZATION: Process only 1 out of every 10 frames
        # This reduces CPU usage by 90% while still catching eye movements
        if self.frame_count % 10 != 0:
            return frame
        
        # Also limit processing to once every 0.3 seconds (3 FPS max)
        ...
        try:
            ...
            if results.multi_face_landmarks:
                ...
                # Detect regressions (backward eye movements)
                if self.last_x - current_x > 0.15: 
                    self.regressions += 1
                    
                # Detect fixations (eyes staying in place)
                if abs(self.last_x - current_x) < 0.03:
                    self.fixations += 1
                    
                self.last_x = current_x
                
                # Draw tracking dots (green)
                cv2.circle(img, (int(left_iris.x * w), int(left_iris.y * h)), 5, (0, 255, 0), -1)
                cv2.circle(img, (int(right_iris.x * w), int(right_iris.y * h)), 5, (0, 255, 0), -1)
                
            # Update session state
            risk = min(99, int(self.regressions * 10))
            st.session_state.metrics["regressions"] = self.regressions
            st.session_state.metrics["fixations"] = self.fixations
            st.session_state.metrics["risk_score"] = risk
...
# 4. UI LAYOUT
# -----------------------------------------------------------------------------
st.markdown('<h1 style="text-align:center;">👁️ Akshar</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align:center; color:#a0a0b0;">Dyslexia Screening via Eye Movement Analysis</p>', unsafe_allow_html=True)

col1, col2 = st.columns([1.5, 1], gap="large")

with col1:
    st.markdown('<div class="embossed-card">', unsafe_allow_html=True)
    
    webrtc_ctx = webrtc_streamer(
        key="akshar-tracker",
        mode=WebRtcMode.SENDRECV,
        video_transformer_factory=VideoTransformer,
        media_stream_constraints={"video": {"width": 640, "height": 480}},  # Lower resolution = less CPU
    )
    ...
    with btn_col1:
        if st.button("▶ Start Session", use_container_width=True, type="primary"):
            ...
            st.session_state.metrics = {"regressions": 0, "fixations": 0, "risk_score": 0, "start_time": time.time()}
            st.rerun()
    with btn_col2:
    ...
    <div class="embossed-card" style="margin-top: 15px;">
        <b>📖 Reading Task:</b><br>
        "The quick brown fox jumps over the lazy dog. Reading requires smooth eye movements across the page..."
    </div>
...
# -----------------------------------------------------------------------------
# 5. SAFE AUTO-REFRESH (Only when active)
# -----------------------------------------------------------------------------
if st.session_state.session_active and webrtc_ctx.state.playing:
    time.sleep(2.0)  # Update UI only every 2 seconds (reduces CPU)
